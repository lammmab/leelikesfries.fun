...
const TAU = Math.PI * 2; // TAU is equivalent to 2 * Math.PI
...
function rand(max) {
  return Math.random() * max; // Returns a random number between 0 and max
}

function fadeInOut(life, ttl) {
  const alpha = life < ttl / 2 ? life / (ttl / 2) : (ttl - life) / (ttl / 2);
  return Math.max(0, Math.min(1, alpha)); // Clamp the value between 0 and 1
}
